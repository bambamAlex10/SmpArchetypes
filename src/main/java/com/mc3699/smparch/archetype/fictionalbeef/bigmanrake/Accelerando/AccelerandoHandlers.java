...
public class AccelerandoHandlers {

    @SubscribeEvent
    public static void onPlayerTick(PlayerTickEvent.Post event) {
        Player player = event.getEntity();

        if (player.level().isClientSide() || !player.hasData(BigManRakeAttachments.HITBOXHIT))
            return;
        ...
        for (LivingEntity gotHit : player.level().getEntitiesOfClass(LivingEntity.class, hitbox)) {
            ...
            if (AccelerandoAbility.firstHit(player, gotHit)) {
