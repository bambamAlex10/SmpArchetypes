# NOTICE
## THIS IS A FORK OF SMP ARCHETYPES!!!!!

This is for my server Fictional Beef, which is why the archetypes folder splits into 2 one for TBS (the broken script server) and FictionalBeef (fictionalbeef)
There are new archetypes for the following individuals:

BigManRake: Lobotomy Corp Slop

  Accelerando - dash with ability to spread his favorite effect (insanity),
	
  AccesoMorendo - passive that makes it so he can stay alive for some extra few important moments.
	
  Concerto - ultimate that can only be used on low health, lobotomy corp slop.
...
AliveAlex1027: My Kit
  ...
  BloodOfCovenant - player can only do damage with a weapon inhand, otherwise any damage will heal the victim.
	
  Retribution - Counter which on success gives 10 seconds of an iframe, on failure teleports player to the void for 10 seconds keeping them at 2 and a half hearts.
...
Scapune: Terrorist Kit
  ...
  DecieversFury - active skill that gives the next highest level of strength for a user. at the highest levels of strength a user may get it for less then 4 seconds.
...
Ykorio: Dark Gambling Kit
  ...
  VoidDomain - passive, scales the users strength to the amount of light they are exposed to. In darker areas they move faster and hit harder, but lighter areas weaken them.
	
  VoidEssence - active, essentially a shield that blocks 2 hits. 
...
JKMC: The Other Terrorist Kit
  ...
  RezeDeath - Passive that expels all gained meter on death into a large explosion.
...
FlameK1ng_: Flame Gambling Kit
  ...
  Brimstone - passive that makes it so that the user can flip coins and heals from fire & explosive damage.
	
  InvertedScale - active counter that depending on the result of a coin flip is longer or shorter in both its hit window and duration of its effect. if the counter is hit the user gets 20% stronger attacks and takes 20% less damage for up to 14 seconds on a head.
	
  Pyrophagia - toggle passive that when activated eats hunger but makes it so the user heals from fire damage done to other entities then themself.
